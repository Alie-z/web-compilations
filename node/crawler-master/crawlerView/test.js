...
async function test() {
  try {
    await driver.get('https://myvid.download/?url=https://www.youtube.com/watch?v=KqjgLbKZ1h0');
  ...
  try {
    ...
    for (let i = 0; i < items.length; i++) {
      try {
        ...
        if (title.indexOf('查看全部') >= 0 || title.indexOf('投票') >= 0) {
          continue
...
//         }
        ...
        console.log(imgsUrl)
        await Promise.all(imgsUrl.map(url => download('https:' + url, '../weiboImg').then(async () => {
          console.log('done!');
        }).catch((e) => {
          console.log('下载出错了-------')
        })));
